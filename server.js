...
function getBuymaCategory(productType, tags) {
...
function mapShopifyToBuyma(sp, overrides = {}) {
  ...
  // ── Category detection ────────────────────────────────────────────────────
  const categoryId = overrides.category_id || getBuymaCategory(sp.product_type, sp.tags);
  if (!categoryId) {
  ...
  const options = [];
  let pos = 1;
  sizes.forEach(s  => options.push({ type: 'size',  value: s, master_id: 0,  position: pos++ }));
  colors.forEach(c => options.push({ type: 'color', value: c, master_id: hasColor ? 99 : 99, position: pos++ }));
